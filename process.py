...
def ui_process2(self, cmd, date="220701", sht_num_cnt=0):
    ...
    if v_str_command == Cmd.LOAD_SPR_INFO:
        ...
        if self.rbt1.isChecked():
            ...
            message = "md.make_df_spr_info(self, df, Filter.PATHNER)"
            log(self, message)
            ...
            message = "md.make_df_spr_info(self, df, Filter.OEM)"
            log(self, message)
            ...
            message = "md.make_df_spr_info(self, df, Filter.SPR_NO)"
            log(self, message)
            ...
            message = "md.make_df_spr_info(self, df, Filter.PROJECT)"
            log(self, message)
            ...
            message = "md.make_df_spr_info(self, df, Filter.DATE)"
            log(self, message)

        else:
            df = md.make_df_spr_info(self, df, Filter.NONE)
            message = "md.make_df_spr_info(self, df, Filter.NONE)"
            log(self, message)
    ...
    if v_str_command == Cmd.LOAD_SPR_SUM:
        ...
        df = md.load_spr_sum_excel(self)
        message = "md.load_spr_sum_excel()"
        log(self, message)

        uimd.set_df_table(self, df, desription=11)
        message = "uimd.set_df_table(self,df)"
        log(self, message)

        get_tbColumnRowCount(self)

    if v_str_command == Cmd.SAVE_F_SPR_SUM_EXCEL:
        message = Cmd.SAVE_F_SPR_SUM_EXCEL.name
        log(self, message)

        md.save_filtered_spr_sum_excel(self)
        message = "md.save_filtered_spr_sum_excel()"
        log(self, message)
...
def ui_process3(self, cmd, date="220701", sht_num_cnt=0):
    ...
    if v_str_command == Cmd.SAVE_SUM_SPR_EXCLE:
        ...
        log(self, message)
        bar_timer_on(self, "start", 10)
        md.save_sum_spr_excel(self)
        message = "md.save_sum_spr_excel"
        log(self, message)
    ...
    if v_str_command == Cmd.LOAD_SUM_SPR_LLP:
        ...
        md.load_llp_sum_spr_excel(self)
        message = "md.load_llp_sum_spr_excel(self)"
        log(self, message)
...
def get_tbColumnRowCount(self):
    cntrow = self.tbw1.rowCount()
    cntcol = self.tbw1.columnCount()
    cowrow = f"열 : {cntrow}, 컬럼 : {cntcol}"
    log(self, cowrow)
