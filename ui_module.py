...
try:
            ...
            for i in range(0, len(jobs)):
                ...
                for j in range(0, len(jobs[i])):
                    ...
                    for k in range(0, len(jobs[i][j].columns)):
                        ...
                        self.tbw[i].setItem(
                            j, k, QTableWidgetItem(str(jobs[i][j].iloc[0, k]))
                        )
                        # 가운데 정렬
                        self.tbw[i].item(j, k).setTextAlignment(
    ...
    # comment :  PLAN 에 통합시트시간, 업무시간, 학습 및 나머지 시간을 읽어와서 TABLE 에 뿌려준다.
    ###############################################################################################
    def set_df_table_2_arr(self, jobs, desription=8):
        try:
            # for k in range(0,len(self.tbw)):
            # self.tbw1.setRowCount(len(jobs.index))
            # self.tbw1.setColumnCount(len(jobs.columns))
            # self.tbw1.setHorizontalHeaderLabels(jobs.columns)
            print("INSIDE")

            # for i in range(len(jobs.index)):
            #     for j in range(len(jobs.columns)):
            #         self.tbw.setItem(i,j,QTableWidgetItem(str(jobs.iloc[i, j])))
            #         # 가운데 정렬
            #         self.tbw.item(i, j).setTextAlignment(Qt.AlignCenter | Qt.AlignVCenter)

            # for k in range(0,len(self.tbw)):
            # 폭 자동조절
            # self.tbw.resizeColumnsToContents()
            # 날짜 폭 고정조절
            # self.tbw.setColumnWidth(2, 85)
            # 좌측 정렬
            # for j in range(len(job.index)):
            # self.tbw1.item(j,desription).setTextAlignment(Qt.AlignLeft | Qt.AlignVCenter)
    ...
    ###############################################################################################
    def set_df_table(self, job, desription=8):
        try:
            ...
            for i in range(len(job.index)):
                for j in range(len(job.columns)):
                    ...
                    # 가운데 정렬
                    self.tbw1.item(i, j).setTextAlignment(
                        Qt.AlignCenter | Qt.AlignVCenter
                    )
            ...
            # Description 좌측 정렬
            for j in range(len(job.index)):
                self.tbw1.item(j, desription).setTextAlignment(
                    Qt.AlignLeft | Qt.AlignVCenter
                )
            # Description 폭 고정조절
            self.tbw1.setColumnWidth(desription, 85)
